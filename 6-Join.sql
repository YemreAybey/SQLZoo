...
/* 7-List the player for every goal scored 
...
/* 8-Instead show the name of all
    players who scored a goal against Germany.*/
...
SELECT matchid,mdate, COUNT(*) as total_goals
  FROM game JOIN goal ON matchid = id 
    WHERE teamid = 'GER' GROUP BY matchid, mdate
     ...
     /* 13-List every match with the goals scored by 
     each team as shown. This will use "CASE WHEN" 
     which has not been explained in 
